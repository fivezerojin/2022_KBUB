...
#define DEBUG 0
#define MODE 1 // 1 : 본선, 2 : 예선, 3 : test, 4:장애물 테스트
using namespace std;
...
void VisionDistance_cb(const std_msgs::Float64::ConstPtr &msg);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber visionDistance = n.subscribe("/stopline", 10, VisionDistance_cb);//정지선 거리(int16)
...
void odomPub(ros::Publisher odom_pub)
{
    ...
    if ((listOfCoord.coordList[cnt][min_index + 1][0] == 1.1 && cnt < listOfCoord.end) && cnt != 0)
    {
...
void setMission(ros::Publisher missionPub)
{
    ...
    */
    ...
    if(MODE == 1)
    {
        ...
        else if(cnt == 5 || cnt == 8 ||cnt==12|| cnt == 18 || cnt==22 || cnt == 27|| cnt ==29|| cnt ==32)    Mission = 1;
        else if(cnt == 10 || cnt == 99) Mission = 4;
        else if(cnt == 13 || cnt == 98) Mission = 7;
        else if(cnt == 17 || cnt == 97) Mission = 8;
        else if(cnt ==24)   Mission = 2;
        else Mission = 0;
    ...
    // }
        ...
        // if(cnt==2 || cnt == 98) Mission =7;
        // else if(cnt == 4 || cnt == 97) Mission=8;
        // else Mission=0;
        // if (cnt == 2 || cnt ==98) Mission = 7;
...
void MissionControl()
{
    ...
    */
        //if (stoplineDistance < 5.0 && inputData->getVisionClass(2)) 
        if(cnt == 18){
            if(stoplineDistance < 8.0 && (inputData->getVisionClass(0) || inputData->getVisionClass(2)) ) 
                EStop();
            else
            {
                SetDrive();
        ...
        {
            if(stoplineDistance < 8.0 && inputData->getVisionClass(0) ) 
                EStop();
            else
            {
                SetDrive();
    ...
    * 정지선이 일정 거리 아래로 감지되면서(getStopDistance), 신호등이 빨간불일경우(getVisionClass(2)) Estop()
    */
        if (stoplineDistance < 8.0 && (inputData->getVisionClass(0) || inputData->getVisionClass(1))) 
            EStop();
        else
        {
            SetDrive();
