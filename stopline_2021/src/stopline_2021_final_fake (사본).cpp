...
int main(int argc, char **argv)
{
	...
	while (ros::ok())
	{
		if(callback == true)
		{
			...
			Scalar lower_white = Scalar(0, 0, 220);    //220
			...
			Mat img_mask;
			img_mask = mask_filter(img_integral, 5, 5, 120); // 5,5,85
			//imshow("img_mask", img_mask);

			Mat warp_inv;
			warp_inv = bird_eyes_view_inverse(img_mask);
			// imshow("warp_inv", warp_inv);o

			Mat img_resize1;
			cv::resize(img_color, img_resize1, cv::Size(641, 481), 0, 0);
			//imshow("frame2", frame2);
			Mat final;
			addWeighted(warp_inv, 0.5, img_resize1, 0.5, 0, final);
