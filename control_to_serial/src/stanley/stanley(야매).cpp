...
public:
    ...
    void Calculate()
    {
        ...
        while( ind < (num_of_cx - 4))
        {
            ...
            double ind_tmp2 = atan2(cy[ind + 3] - cy[ind+1], cx[ind+3] - cx[ind+1]);
            
            // double ind_tmp1 = atan2(cy[ind + 4] - cy[ind+3], cx[ind+4] - cx[ind+3]);
            // double ind_tmp2 = atan2(cy[ind + 3] - cy[ind+2], cx[ind+3] - cx[ind+2]);
            // double ind_tmp3 = atan2(cy[ind + 2] - cy[ind+1], cx[ind+2] - cx[ind+1]);

            // double ind_tmp4 = atan2(cy[ind + 8] - cy[ind+2], cx[ind+8] - cx[ind+2]);
            // double ind_tmp5 = atan2(cy[ind + 7] - cy[ind+1], cx[ind+7] - cx[ind+1]);

            

            // printf("cy[ind] :%lf, cy[%d+3] : %lf\n", cy[ind], ind, cy[ind + 3]);
            double ind_tmp = NormalizeAngle((ind_tmp1+ind_tmp2)/2);
            index[ind] = ind_tmp;
            ind += 1;
        }
...
void callback1(const nav_msgs::Odometry::ConstPtr& odom)
{
    ...
    if(Flag)
    {
        ...
        //     k = 1.2 ;
        // }
        // else if(abs(delta) <= 10 && abs(delta) > 5)
        // { 
        //     k = 1.0;
        // }
        // else
        ...
        if(abs(pre_brake_trigger) > 16*(PI/180) && abs(delta) > 10)
        {
            k = 1.5;
        }
        else if((abs(pre_brake_trigger) <= 16*(PI/180) && abs(pre_brake_trigger) > 9*(PI/180)) && (abs(delta) <= 10 && abs(delta) > 5))
        { 
            k = 1.0;
        }
        else
