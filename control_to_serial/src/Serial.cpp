...
//========================== stanley ==========================
void R_callback(const std_msgs::Float32::ConstPtr& float_msgs)	
{
	curv = (float_msgs->data);
}
...
void steer_callback(const std_msgs::Float32::ConstPtr& float_msgs)	
{
	...
	if (del_brake > 50 && parking==false && mission != 10 && mission !=6)
	{
		delta = del_brake - 100;
		brake = 0;
	}
	else if(del_brake<=50 && parking==false && mission != 10 && mission !=6)
	{
		delta = del_brake;
		brake = 0;
	}
	...
	else if (mission == 4) //4 정적대형
	{
		speed = 10;//8
		brake = 0;
	}
	else if (mission == 11) //txt 1번
	{
		speed = 11;
		brake = 0;
	}
...
////////////////////////////////////////////////////////
		if (curv >= 150)
		{
			...
			else if (abs(delta) > 6 && abs(delta) < 8)
			{
				speed = 15; //15
			}
...
void theta_callback(const std_msgs::Float64::ConstPtr &float_msgs)     //주차 3단계
{
	...
	else
	{
		...
		else if (theta < 65 && theta >= 60) //60~65
		{
			delta = 24; //28
			speed = 8; //6 //중요
			brake = 0; //0
...
void theta2_callback(const std_msgs::Float64::ConstPtr &float_msgs)     //주차 4단계
{
	...
	if (stopline_parking > 2)
	{
		...
		// 	brake = 0;
		// }
		// else if (theta2 < 15 && theta2 > 13) //8~15
		// {
		// 	delta = 8; //10
		// 	speed = 5;	//8
		// 	brake = 0;	//0
		// }
		// else if (theta2 < 13 && theta2 > 10) //8~15
		// {
		// 	delta = 7; //10
		// 	speed = 5;	//8
		// 	brake = 0;	//0
...
int main(int argc, char **argv)
{
	...
	ros::Subscriber R_sub = n.subscribe<std_msgs::Float32>("/Control/R", 1, R_callback);
