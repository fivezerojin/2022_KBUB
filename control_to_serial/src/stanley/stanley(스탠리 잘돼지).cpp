...
#define NUM_OF_C 15 //7976 //txt파일 라인개수 + 1 //점 개수에 따라 수정
...
public:
    ...
    void Calculate()
    {
        ...
        while( ind < (num_of_cx - 3))
        {
            // double ind_tmp = atan2(cy[ind + 5] - cy[ind+4], cx[ind+5] - cx[ind+4]);
            double ind_tmp = atan2(cy[ind + 3] - cy[ind+2], cx[ind+3] - cx[ind+2]);
            printf("cy[ind] :%lf, cy[%d+3] : %lf\n", cy[ind], ind, cy[ind + 3]);
            ind_tmp = NormalizeAngle(ind_tmp);
            index[ind] = ind_tmp;
            ind += 1;
        }

        if(targetIndex < num_of_cx - 1)
        ...
        if(targetIndex < num_of_cx - 6)
        {
            double cur_tan = atan2(cy[targetIndex + 1] - cy[targetIndex], cx[targetIndex + 1] - cx[targetIndex]);
            double pre_tan = atan2(cy[targetIndex+5] - cy[targetIndex + 4], cx[targetIndex + 5]-cx[targetIndex + 4]);
            double brake_triger = pre_tan - cur_tan;
            if(abs(brake_triger) > 10*(PI/180))
                {
                    pre_brake = true;
                }
            else            
...
void callback1(const nav_msgs::Odometry::ConstPtr& odom)
{
    ...
    if(Flag)
    {
        ...
        yaw_data = cal_yaw + pre_del * (PI/180) * 0.4;
        double e_data = e;
