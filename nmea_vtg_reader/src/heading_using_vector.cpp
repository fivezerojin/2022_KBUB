...
static int first = 1;
static int second = 1;
int str1 = 1,str2 = 1,str3 =1,str4=1, str5 = 1, str6 = 1, str7 = 1, str8 = 1, str9 = 1, str10 = 1,str11=1;
ros::Time t1, t2;
...
void heading_callback(const nav_msgs::Odometry::ConstPtr &odom)
{
    ...
    if (first > 9 && vecx_saved[0] != 0 && vecy_saved[0] != 0)
    {
        ...
        if (GetHeadingAngle(v1, VectorSubtractVector(v3, v2)))
        {
            ...
            // NaN 처리
            if (isnan(heading.data)!=0){
                ROS_WARN("---------------NaN detected!!----------------");
                heading.data = result_save[0];
            }
            else{
                result_temp = heading.data;
